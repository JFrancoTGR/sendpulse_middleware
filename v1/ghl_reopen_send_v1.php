...
/**
 * GHL Reopen Send V1 — DRY RUN ONLY.
 *
 * Fuente:
 ...
 *
 * IMPORTANTE:
 * - DRY RUN ONLY: no contiene POST a sendTemplateByPhone.
 * - El ledger global se consulta pero NO se modifica en dry run.
 * - No modifica GHL ni SendPulse.
 *
 * Acciones HTTP:
...
switch ($action) {
    ...
    default:
        respondAndExit([
            ...
            'allowed_actions' => ['create_run', 'process_batch', 'status'],
            'examples' => [
                '?action=create_run&source_run_id=RUN_ID',
                '?action=process_batch&send_run_id=RUN_ID&batch_size=10',
                '?action=status&send_run_id=RUN_ID',
...
function processSendBatchAction(
    ...
    try {
        ...
        if (($manifest['dry_run_only'] ?? null) !== true) {
            throw new RuntimeException('El manifest no está bloqueado en dry run.');
        }
        ...
        logLine(
            ...
            . count($candidateIds)
            . " batch_size={$batchSize} retry_errors="
            . ($retryErrors ? '1' : '0')
            . ' debug=' . ($debug ? '1' : '0')
        );
        ...
        foreach ($candidateIds as $sendItemId) {
            ...
            try {
                ...
                if (($ghlEvaluation['status'] ?? '') !== 'eligible') {
                    ...
                    $sequenceEvaluation['cycle_id_snapshot'] = $item['cycle_id_snapshot'] ?? null;
                ...
                logLine(
                    ...
                    . ' would_send=' . (($resultItem['decision']['would_send'] ?? false) ? '1' : '0')
                );
        ...
        logLine(
            ...
            . ' ready=' . $summary['evaluation']['ready_to_send']
            . ' errors=' . $summary['state']['error']
        );
        logLine(
            $globalLogPath,
            "BATCH_END send_run_id={$sendRunId} elapsed_ms={$elapsedMs} processed={$processedThisBatch} pending="
...
function sendStatusAction(string $sendStorageRoot, string $sendRunId): never
{
    validateRunIdOrExit($sendRunId, 'send_run_id');
    $paths = sendRunPaths($sendStorageRoot, $sendRunId);
    assertFilesExist($paths, ['manifest', 'state', 'results']);
...
function buildLedgerWritePreview(
    ...
    $preview['cycle_id'] = $cycleId;
    $preview['opportunity_id'] = $opportunityId;
    $preview['project_key'] = $projectKey;
    $preview['enfriado_entered_at'] = $lastStageChangeAt;
    $preview['status'] = $preview['status'] ?? 'active';
    $preview['steps'] = is_array($preview['steps'] ?? null) ? $preview['steps'] : [];
    $preview['steps'][(string) $stepNumber] = [
        'status' => 'sent',
        'template' => $templateName,
    ...
    $preview['last_sent_at'] = date(DATE_ATOM);
    $preview['last_step_sent'] = $stepNumber;
    if ($stepNumber >= $finalStepNumber) {
        $preview['status'] = 'sequence_complete';
        $preview['completed_at'] = date(DATE_ATOM);
    }
...
function buildSendSummary(array $manifest, array $state, array $results): array
{
    ...
    ksort($evaluation['would_send_by_project'], SORT_STRING);
    ksort($evaluation['would_send_by_step'], SORT_NATURAL);
    ksort($evaluation['terminal_by_status'], SORT_STRING);
    ksort($evaluation['reason_counts'], SORT_STRING);
