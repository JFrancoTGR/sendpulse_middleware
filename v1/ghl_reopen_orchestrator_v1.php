...
$universeScript = trim((string) (
    $runtime['reopen_automation_universe_script']
    ?? 'ghl_reopen_universe_query_v1.php'
));
$senderScript = trim((string) (
    $runtime['reopen_automation_sender_script']
    ?? 'ghl_reopen_send_live_guarded.php'
));
...
switch ($action) {
    ...
    default:
        orchestratorRespond([
            ...
            'allowed_actions' => ['selftest', 'status', 'run'],
            'cli_examples' => [
                'php ghl_reopen_orchestrator_v1.php action=selftest',
                'php ghl_reopen_orchestrator_v1.php action=status',
                'php ghl_reopen_orchestrator_v1.php action=run',
...
function orchestratorRun(
    ...
    try {
        ...
        if (($state['active_cycle']['phase'] ?? '') === 'universe') {
            ...
            while (true) {
                if (elapsedSeconds($startedAt) >= $maxRuntimeSeconds) {
        ...
        if (($state['active_cycle']['phase'] ?? '') === 'send') {
            $sendRunId = (string) $state['active_cycle']['send_run_id'];
            ...
            while (true) {
                if (
                    ...
                    orchestratorRespond([
                        ...
                        'active_cycle' => $state['active_cycle'],
                        'metrics' => $metrics,
                        'message' => 'El siguiente cron reanudará el mismo send_run; no se crea otro ciclo.',
                    ], 200);
                }
                ...
                if ($pendingItemId === null) {
                    ...
                    if (($status['json']['complete'] ?? false) !== true) {
                        throw new RuntimeException('No hay pending items pero el sender no reporta complete=true.');
                    }
                    ...
                    $state['last_summary'] = [
                        'universe_run_id' => $state['active_cycle']['universe_run_id'] ?? null,
                        'send_run_id' => $sendRunId,
                    ...
                    orchestratorRespond([
                        ...
                        'mode' => 'orchestrator_run',
                        'status' => 'complete',
                        'side_effects' => ((int) ($summary['messages_sent'] ?? 0)) > 0,
                ...
                // Regla de seguridad: una falla/timeout de la llamada live NO se reintenta.
                ...
                if (($send['json']['ok'] ?? false) !== true) {
                    blockOrchestrator(
                        ...
                        details: [
                            ...
                            'response_reason' => $send['json']['reason'] ?? null,
                            'response_error' => $send['json']['error'] ?? null,
                        ]
                    );
                    throw new OrchestratorBlockedException(
                ...
                if (($send['json']['mode'] ?? '') !== 'process_send_batch') {
                    throw new RuntimeException('Respuesta inesperada del sender live.');
                }

                $processed = (int) ($send['json']['processed_this_batch'] ?? 0);
                if ($processed !== 1) {
                    throw new RuntimeException('Sender live no procesó exactamente un item explícito.');
                }

                $metrics['send_items_processed'] += 1;
                $metrics['messages_sent'] += (int) ($send['json']['messages_sent'] ?? 0);
                ...
                $state['active_cycle']['last_progress_at'] = date(DATE_ATOM);
                $state['active_cycle']['last_send_item_id'] = $pendingItemId;
                $state['updated_at'] = date(DATE_ATOM);
                writeOrchestratorJson($statePath, $state);
            }
        }
